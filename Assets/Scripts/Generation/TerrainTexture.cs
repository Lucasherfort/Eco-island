...
public class TerrainTexture : MonoBehaviour {
    ...
    public void SetTexture(Terrain terrain, AnimationCurve meshHeightCurve, List<TerrainType> regions) {
        ...
        List<TerrainType> listTextures = regions;
        ...
        for (int y = 0; y < terrainData.alphamapHeight; y++)
        {
            for (int x = 0; x < terrainData.alphamapWidth; x++)
            {
                ...
                for (int i = 0; i < listTextures.Count; i++) {
                    ...
                    if (normHeight >= meshHeightCurve.Evaluate(listTextures[i].minAltitude) && normHeight <= meshHeightCurve.Evaluate(listTextures[i].maxAltitude) && normSteepness >= listTextures [i].minSteepness && normSteepness <= listTextures [i].maxSteepness) {
