...
namespace UnityEditor.AI
{
    public class NavMeshAssetManager : ScriptableSingleton<NavMeshAssetManager>
    {
        ...
        static void CreateNavMeshAsset(NavMeshSurface surface)
        {
            ...
            var combinedprefabAssetPath = Path.Combine(targetPath, "NavMesh-" + surface.name + ".asset");
            combinedprefabAssetPath = AssetDatabase.GenerateUniqueAssetPath(combinedprefabAssetPath);
            AssetDatabase.CreateAsset(surface.navMeshData, combinedprefabAssetPath);
        }
        ...
        void DeleteStoredPrefabNavMeshDataAsset(NavMeshSurface surface)
        {
            for (var i = m_PrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
            {
                ...
                if (storedAssetInfo.surface == surface)
                {
                    ...
                    {
                        var prefabAssetPath = AssetDatabase.GetAssetPath(storedNavMeshData);
            ...
            do
            {
                ...
                for (var i = m_PrefabNavMeshDataAssets.Count - 1; i >= 0; i--)
                {
                    ...
                    if (storedPrefabInfo.surface == null)
                    {
                        ...
                        {
                            var prefabAssetPath = AssetDatabase.GetAssetPath(storedPrefabInfo.navMeshData);
                            AssetDatabase.DeleteAsset(prefabAssetPath);
                        }
                    ...
                    else if (surfaceInPrefab != null && storedPrefabInfo.surface == surfaceInPrefab)
                    {
                        ...
                        {
                            var prefabAssetPath = AssetDatabase.GetAssetPath(surfaceInPrefab.navMeshData);
                            AssetDatabase.DeleteAsset(prefabAssetPath);
                            ...
                            //    surfaceInPrefab, prefabAssetPath);
                        }
